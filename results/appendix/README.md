...
Deliverables of the appendix analyses. The scripts that produce them live in
`scripts/appendix/` and take their paths from `cues/paths.py`, so they run from
any working directory:

```bash
python scripts/appendix/<script>.py
```

Inputs come from `data/plot_data/`, `results/regressions_all_models/` (build it first,
see the top-level README) and the bundled aggregates `per_seed_agg.parquet` /
