...
compared against what the appendix currently prints:
  ...
  Check 1 -- regression significance counts (paper Sec. 4 / Tables reg_*):
    ...
    7/9 (inferred race) and 9/9 (readability).
      Sources: LLaMA-3.1 -> results/regressions_llama/{medical,legal,salary}/
                             side_by_side_coefficients.csv (col "(3)")
               GPT-5.2/OLMo-2 -> results/regressions_all_models/{gpt52,olmo2}/
  ...
  Check 2 -- Black-recall-by-cue table (Table black_recall_by_model):
    ...
    results/race_inference/black_recall_wide.csv.
...
REG_SRC = {
    "LLaMA-3.1": lambda t: f"{REGRESSIONS_LLAMA}/{t}/side_by_side_coefficients.csv",
    "GPT-5.2":   lambda t: f"{REGRESSIONS_ALL_MODELS}/gpt52/{TASK_ALL[t]}/side_by_side.csv",
    "OLMo-2":    lambda t: f"{REGRESSIONS_ALL_MODELS}/olmo2/{TASK_ALL[t]}/side_by_side.csv",
}
...
rec_df = pd.read_csv(f"{RACE_INFERENCE}/black_recall_wide.csv")
